...
function generateTimetable() {
  ...
  const sections = Object.keys(sectionTimings);
  const randomSection = sections[Math.floor(Math.random() * sections.length)];
  const timings = sectionTimings[randomSection];

  const table = `
    <h4 class="mb-3">🗓️ Timetable for ${course} - ${classYear}, Section ${randomSection}</h4>
